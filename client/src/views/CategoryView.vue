  ...
  <div class="relative">
    ...
    <div v-else-if="categoryStore.currentCategory" class="max-w-4xl mx-auto">
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <h2 class="text-lg font-semibold text-gray-900 mb-3">{{ t('categoryView.sectionHeading') }}</h2>

        <div v-if="categoryPrompts.length === 0" class="text-gray-500 italic">
          {{ t('categoryView.noPromptsInCat') }}
        </div>
        ...
        <p class="text-xs text-gray-400 mt-4">
          {{ t('date.created') }} {{ new Date(categoryStore.currentCategory.createdAt).toLocaleDateString() }}
        </p>
      </div>
